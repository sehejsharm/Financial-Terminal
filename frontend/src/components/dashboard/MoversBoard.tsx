...
export function MoversBoard() {
  ...
  const gain = useLive<Mover[]>(
    () => api.movers("gainers", 10).then((r) => (Array.isArray(r) ? r : [])), 60_000, []);
  const lose = useLive<Mover[]>(
    () => api.movers("losers", 10).then((r) => (Array.isArray(r) ? r : [])), 60_000, []);

  return (
    <div>
      <div className="flex items-center gap-2 mb-2">
        <h2 className="heading">Gainers &amp; losers</h2>
        <span className="text-[10px] text-mut">NIFTY constituents</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        <Column kind="gainers" rows={gain.data ?? []} busy={gain.busy} />
        <Column kind="losers" rows={lose.data ?? []} busy={lose.busy} />
      </div>
    </div>
  );
}
